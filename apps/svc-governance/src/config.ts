...
const configSchema = z.object({
  port: z.coerce.number().min(1000).max(65535).default(3005),
...
export type GovernanceConfig = z.infer<typeof configSchema>;

export const governanceConfig: GovernanceConfig = configSchema.parse({
  port: process.env.GOVERNANCE_SERVICE_PORT || process.env.PORT || 3005,
